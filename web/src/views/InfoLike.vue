<template>
  <div class="follower">
    ...
    <div v-if="list.length>0" class="bg-white">
      ...
      <ul>
        <li class="d-flex jc-between px-3 mx-1 py-3 ai-center fs-lg text-grey-1 border w-100" v-for="(item, index) in list" :key="index">
          ...
          <span class="d-flex ml-2" @click="$router.push(`/trends/${item.content}`)">点赞了你的动态</span>
    ...
    <van-empty class="bg-white" v-else description="暂无点赞消息~" />
...
export default {
  ...
  methods: {
    handleTime,
    async init() {
      const { data } = await this.$http.get(`/info/likes/${this.id}`)
